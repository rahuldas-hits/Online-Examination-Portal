...
<?php
    ...
    if (isset($_POST['review'])) {
        ...
        $question_id = $_POST['review_qsn_id'];
        $review_counter = $_POST['review_counter'];
        $total_qsn = $_POST['totalqsn'];
        
        $review_list = array();

//        $_SESSION['review_list'] =  $review_list;
        

        $review_list = $_SESSION['review_list']; 
        
        if ($review_counter > 0 || $review_counter < 20) {
            echo $review_counter = $review_counter + 1;                
            $_SESSION['review_counter'] = $review_counter;    
        }else if ($review_counter > 20){
            $review_counter = 0;                
            $_SESSION['review_counter'] = $review_counter;    

        }
        
        for($review_counter; $review_counter <10 ; $review_counter++) { 
            $review_list[$review_counter] = $question_id;
            break;
        }
            ...
            <div class="pcoded-wrapper">
                <div class="">
                    <div class="pcoded-inner-content">
                        <?php
                        @$testid = $_SESSION['selected_test_id'];
                        if($cnt == 0){
                            ...
                            $sql = "SELECT * from test a INNER JOIN question b WHERE test_id = $testid AND a.id = b.test_id limit 1 OFFSET $cnt";
                            /*echo $sql = "SELECT * FROM question WHERE test_id = $demotestid limit 1 OFFSET $cnt";*/
                            $res = mysqli_query($conn,$sql);
                            $row = mysqli_fetch_array($res);
                            $totalquestion = $row['total_question'];
                            $review_question = $row['id'];
                    ...
                    <?php
                            ...
                            $sql = "SELECT * from test a INNER JOIN question b WHERE test_id = $test_id AND a.id = b.test_id ;";
                            $res = mysqli_query($conn,$sql);
                            $count_row_number = mysqli_num_rows($res);
                            $row = mysqli_fetch_array($res);
                            
                            $test_id = $_SESSION['selected_test_id'];
                            echo $sql = "SELECT * from test a INNER JOIN question b WHERE test_id = $test_id AND a.id = b.test_id LIMIT 1 OFFSET $page_1;";
                            $res = mysqli_query($conn,$sql);
                            $row = mysqli_fetch_array($res);
                            $review_question = $row['id'];
                                                    ...
                                                    <nav aria-label="Page navigation example">
                                                        <div class="col-sm-12 col-xl-9">
                                                            ...
                                                            <div class="form-group row">
                                                                ...
                                                                <?php 
                                                                    $review_counter =$_SESSION['review_counter'];
                                                                    if ($review_counter == 0) {
                                                                       echo "check if review counter is zero".$review_counter = 0; 
                                                                    }else{
                                                                        echo "if not zero ".$review_counter;
                                                                    }
                                                                ...
                                                                <div class="col">
                                                                    <div class="btn-group " role="group" data-toggle="tooltip" data-placement="top" title="" data-original-title=".btn-md">
                                                                        ...
                                                                        <button type="submit" name="review" class="btn btn-primary btn-sm waves-effect waves-light btn-skew">Mark for review</button>
                                                                        <input type="hidden" name="review_qsn_id" value="<?= $review_question ?>" />
                                                                        <input type="hidden" name="review_counter" value="<?= $review_counter ?>" />
                                                    ...
                                                    <div class="card-block">
                                                        <nav aria-label="Page navigation example">
                                                            ...
                                                            <?php
                                                                ...
                                                                $review_list = $_SESSION['review_list'];
                                                                foreach ($review_list as $value) {
                                                                    echo "ghfjhgvbjk "."$value"."is as follow ";
                                                                }
                                                ...
                                                <div class="card">
                                                    ...
                                                    <div class="card-block">
                                                        <nav aria-label="Page navigation example">
                                                            ...
                                                            ?></strong></h5><hr>
                                                                <?php 
                                                                    for ($review_counter=0; $review_counter <$count_row_number ; $review_counter++) { 
                                                                              echo "<button class='btn btn-info btn-round btn-sm'><a href ='test.php?page={$review_counter} '>{$review_counter}</a></button>";
