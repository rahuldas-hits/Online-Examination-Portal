...
<?php 
  ...
  if (isset($_POST['addqsn'])) {
    ...
    if ($_SESSION['capnum'] < $counter) {       
    //sql query for entrying question
    $sql1 = "INSERT INTO `question`(`category_id`, `sub_category_id`, `test_id`, `question`, `opt1`, `opt2`, `opt3`, `opt4`, `correct_answer`) VALUES ($category,$subCategory,$testName,'$qsn','$opt1','$opt2','$opt3','$opt4',$correctopt)";
    ...
    echo "    
        ...
        echo "    
           <div class='alert alert-success alert-dismissible'>
              <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
              <strong>Success!</strong> added all question for this paper.
            </div>
        ";   
    }
                      ...
                      <form class="form-horizontal style-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                          <div class="form-group">
                              <div class="col-sm-10">
                                  ...
                                  <?php 
                                      $sql = "SELECT a.id , b.id ,a.category FROM category a INNER JOIN test b WHERE b.id = $subid";
                                      $res = mysqli_query($conn,$sql);        
                                      $row = mysqli_fetch_array($res); 
                                  ?>
                                  <select name="category" class="form-control" id="focusedInput">
                                    <option value="<?php echo $row['id'] ?>"><?php echo $row['category'] ?></option>
                                    <option value="">Select Category for test </option>
                                  </select>
                              </div>
                          </div>
                          <div class="form-group">
                              <div class="col-sm-10">
                                  <!-- <input class="form-control" id="focusedInput" type="selects" placeholder="Select Quiz title"> -->
                                  <?php 
                                      $sql = "SELECT a.id , b.id ,a.sub_category_name  FROM sub_category a INNER JOIN test b WHERE b.id = $subid;";
                                      //$eid = $row[eid];
                                      $res = mysqli_query($conn,$sql);        
                          ...
                          <div class="form-group">
                              <div class="col-sm-10">
                                  <!-- <input class="form-control" id="focusedInput" type="selects" placeholder="Select Quiz title"> -->
                                  <?php 
                                      $sql = "SELECT a.id , b.id ,a.Test_name  FROM test a INNER JOIN test b WHERE b.id = $subid;";
                                      //$eid = $row[eid];
                                      $res = mysqli_query($conn,$sql);        
                                    while ($row = mysqli_fetch_array($res)):; 
                                  ?>
                          ...
                          <?php  
                              $sql = "SELECT * FROM test a INNER JOIN question b WHERE a.id = 2 ;";
                              //$eid = $row[eid];
                              $res = mysqli_query($conn,$sql);        
                              $row = mysqli_fetch_array($res); 
                          ...
                          <div class="form-group">
                              <div class="col-sm-10">
                                  <input class="form-control" name="question" id="focusedInput" type="text" placeholder="Enter Question>" value="<?php echo $row['question'] ?>">
                              </div>
                          </div>
                          <div class="form-group">
                              <div class="col-sm-10">
                          ...
                          <center><button type="submit" name="addqsn" value="<?= $count ?>" class="btn btn-theme btn-primary btn-lg btn-block">Submit </button></center>
                      </form>
                  </div>
              </div><!-- col-lg-12-->       
            </div><!-- /row -->
